...
var GREENNAV = GREENNAV || {};

GREENNAV.NeuralCar = function NeuralCar (backCanvas, frontCanvas, console, settings, board) {
	setTimeout(function () {
		window.onbeforeunload = function () {
			return "Careful: It will result in loss of work.";
		};
	}, 5 * 60 * 1000);
	this.setSettings = function defaultSettings (s) {
		...
		settings.cars = parseInt(s.cars) || 75;
		settings.stepSize = parseInt(s.stepSize) || 1000 / 20;
		settings.generationTime = parseInt(s.generationTime) || (16 - 8/2) * 1000;
		...
		settings.car.length = parseInt(s.car.length) || 30;
		settings.car.color = s.car.color || settings.car.colour || "orange";
		...
		settings.board.streetWidth = parseFloat(s.board.streetWidth) || 5;
	...
	GREENNAV.RegexBrain.prototype = new GREENNAV.Brain({}, settings);

	setRegexBrainPrototype();
	
	this.console = new GREENNAV.Console(console);
	this.board = new GREENNAV.Board(board, settings, this);
	this.carCollection = new GREENNAV.CarCollection([], settings, this);
	this.screen = new GREENNAV.Screen(backCanvas, frontCanvas);
	
	this.screen.drawBackground(this.board);
	
	if (settings.debugging.playerCar) {
		var playerCar = new GREENNAV.Car(new GREENNAV.PlayerInput(), settings);
...
function setRegexBrainPrototype () {
	GREENNAV.RegexBrain.prototype.getInput = function getInput (car, board, cars) {
	...
	GREENNAV.RegexBrain.prototype.mutate = function mutate (regexes, mutationRate) {
		var copys = {};
	...
	GREENNAV.RegexBrain.prototype.mutatedRegExp = function mutatedRegExp (regex, mutationRate) {
	...
	GREENNAV.RegexBrain.prototype.getNetwork = function getNetwork () {
		return this.regexes;
	};

	GREENNAV.RegexBrain.prototype.setNetwork = function setNetwork (regexes) {
		this.regexes = regexes;
	};
}

GREENNAV.Brain = function Brain (network, settings, neuralCarInstance) {
...
GREENNAV.RegexBrain = function RegexBrain (network, settings, neuralCarInstance) {
	this.regexes = {
...
GREENNAV.PlayerInput = function PlayerInput () {
	this.keysPressed = {};
...
GREENNAV.Car = function Car (brain, settings) {
	this.score = 0;
	this.bestLength = 0;
	this.lastScore = 0;
	this.velocity = 0;
...
GREENNAV.CarCollection = function CarCollection (carArray, settings, neuralCarInstance) {
	carArray = carArray || [];
	...
	for (var k = 0; k < settings.cars; k++) {
		if (settings.useRegex) {
			this.add(new GREENNAV.Car(new GREENNAV.RegexBrain(undefined, settings, neuralCarInstance), settings), true);
		} else {
			this.add(new GREENNAV.Car(new GREENNAV.Brain(undefined, settings, neuralCarInstance), settings), true);
		}
	}
};

GREENNAV.Board = function Board (board, settings, neuralCarInstance) {
...
GREENNAV.Screen = function Screen (backCanvas, frontCanvas) {
...
GREENNAV.Console = function Console (element) {
